...
function [theta_nodes,er_hat_nodes,Consensus_all_nodes,q_dis_nodes,erm_mean_nodes1,erm_mean_nodes2,erm_mean_nodes3,erm_mean_nodes4,erm_mean_nodes5]=Domkl_online_rf_rev(y,X,params,Connectivity)
...
%parameter
eta=params.eta; % for consensus
eta_g=params.eta_g; % for weight
rho=params.rho;
...
w_nodes = cell(n_nodes,n_ker); % w for each node
...
q_dis_nodes=cell(n_nodes,n_nodes);
...
for t=1:n_data
    ...
    for i = 1:n_nodes
        ...
        for j = 1:n_ker
            ...
            for n=1:length(n_list)
                n_fx=n_fx_nodes{i,n_list(n)};
                if t==1
                    n_w0 = W_int;
                else
                    n_w0 = w_nodes{n_list(n),j};
                end
                n_kx0=[sin(D{n_list(n),j}*x); cos(D{n_list(n),j}*x)];
                n_fx(j)=n_kx0'*n_w0;   
                n_fx_nodes{i,n_list(n)}=n_fx;
            end
            
            % For disconnected OMKL
            %t_nei=t_Nei{i};
            %for n=1:length(t_nei)
            %    n_fx=n_fx_nodes{i,t_nei(n)};
            %    if t==1
            %        n_w0 = W_int;
            %    else
            %        n_w0 = w_nodes{t_nei(n),j};
            %    end
            %    n_kx0=[sin(D{t_nei(n),j}*x); cos(D{t_nei(n),j}*x)];
            %    n_fx(j)=n_kx0'*n_w0;   
            %    n_fx_nodes{i,t_nei(n)}=n_fx;
            %end
            
            
            
            
            % Regression-theta update
            er_temp(j,1)=(y_temp(t)-fx(j))^2; %LOSS_TEMP
            %l_g=-2*(y_temp(t)-fx(j))*kx0; %GRADIENT
            %w_temp= w0 - (eta_l)*l_g - 2*lambda*(eta_l)*w0;
            nei = Nei{i};
            pre_w_sum = 0;
    ...
    for i=1:n_nodes
        ...
        for n=1:length(n_list)
            n_f_hat=n_f_hat_nodes{i,n_list(n)};
            n_fx=n_fx_nodes{i,n_list(n)};
            q=q_nodes{n_list(n)};
            %q=1;
            n_f_hat(t)=n_fx*q;            
            n_f_hat_nodes{i,n_list(n)}=n_f_hat;
            dis=((f_hat(t)-n_f_hat(t)));
            dis_all=dis_all+dis;
        end
        
        Consensus_cons(t)=(dis_all/length(n_list)).^2;
    ...
    %    dis_all=0;
    %    Consensus_cons=Consensus_cons_nodes{i};
    %    all_n_nodes=[1:n_nodes];
    %    all_n_nodes(find(all_n_nodes==i))=[];
    %    for n=1:length(all_n_nodes)
    %        n_f_hat=n_f_hat_nodes{i,all_n_nodes(n)};
    %        n_fx=n_fx_nodes{i,all_n_nodes(n)};
    %        q=q_nodes{all_n_nodes(n)};
    %        %q=1;
    %        n_f_hat(t)=n_fx*q;            
    %        n_f_hat_nodes{i,all_n_nodes(n)}=n_f_hat;
    %        dis=((f_hat(t)-n_f_hat(t))).^2;
    %        dis_all=dis_all+dis;
    %    end
        
    %    Consensus_cons(t)=dis_all;
    %    Consensus_cons_nodes{i}=Consensus_cons;
    %end
    
    %% Assumption4 on weight check
    for i=1:n_nodes
        nei = Nei{i};
        q=q_nodes{i};
        for n=1:length(nei)
            q_dis=q_dis_nodes{i,nei(n)};
            n_q=q_nodes{nei(n)};
            q_dis(t)=sum((q-n_q).^2);
            q_dis_nodes{i,nei(n)}=q_dis;
        end
    end
    ...
    %    theta_up=theta.*theta_nei;
    %    f_hat(t)=fx*(theta_up/sum(theta_up));
        %f_hat(t)=fx;
    ...
    for i=1:n_nodes
        ...
        %if t<round(0.1*n_data)+1
        %    er(t)=0;
        %    erm(t)=0;
        %else
        %    er(t)=(y_temp(t)-f_hat(t))^2;
        %    erm(t)= sum(er)/(t-round(0.1*n_data));
        %end
...
%    erm_mean_nodes6=erm_mean_nodes{6};
%    erm_mean_nodes7=erm_mean_nodes{7};
%    erm_mean_nodes8=erm_mean_nodes{8};
%    erm_mean_nodes9=erm_mean_nodes{9};
